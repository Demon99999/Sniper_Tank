...
namespace Assets.Scripts.Infrastructure.Factoris.TankFactory
{
    public class TankFactory : ITankFactory
    {
        ...
        {
            return await _deskTankWrapperFactory.Create(TankFactoryAssets.DeskTankWrapper, position, parent);
        }
        ...
        public async UniTask<PlayerAccessor> CreatePlayerGlasses(Vector3 position, Quaternion rotation,
            Transform parent)
        {
            return await _playerAccessorFactory.Create(TankFactoryAssets.PlayerGlasses, position, rotation, parent);
        }
        ...
        {
            return await _droneFactory.Create(TankFactoryAssets.Drone, position, rotation);
        }
